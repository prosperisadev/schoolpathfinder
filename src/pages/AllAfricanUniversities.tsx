...
export default function AllAfricanUniversities() {
  ...
  // Get ALL African universities offering this course (EXCLUDING NIGERIA)
  const courseMapping = getUniversitiesForCourse(courseId || "");
  const africanUniversityIds = courseMapping?.africanUniversityIds || [];
  
  // Get ALL rankings (not just top 5)
  const allRankings = getCourseUniversityRankings(courseId || "", "africa");
  ...
  const renderUniversityCard = (ranking: any) => {
    const university = getAfricanUniversityDetails(ranking.universityId);
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">
            All African Universities Offering {course.name}
          </h1>
          ...
          <Card className="bg-yellow-50 border-yellow-200">
            <CardContent className="p-12 text-center">
              <h3 className="text-xl font-semibold text-gray-900 mb-2">
                No African Universities Offer This Course
              </h3>
              <p className="text-gray-600 mb-4">
                {course.name} is not currently offered at African universities outside Nigeria in our database.
              </p>
