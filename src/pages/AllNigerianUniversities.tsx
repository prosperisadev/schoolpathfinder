...
export default function AllNigerianUniversities() {
  ...
  // Get ALL Nigerian universities offering this course
  const courseMapping = getUniversitiesForCourse(courseId || "");
  const nigerianUniversityIds = courseMapping?.nigerianUniversityIds || [];
  ...
  const renderUniversityCard = (ranking: any) => {
    const university = getNigerianUniversityDetails(ranking.universityId);
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">
            All Nigerian Universities Offering {course.name}
          </h1>
          <p className="text-lg text-gray-600">
            Complete list of {validRankings.length} Nigerian {validRankings.length === 1 ? 'university' : 'universities'} where you can study {course.name}
          </p>
        </div>
          ...
          <Card className="bg-yellow-50 border-yellow-200">
            <CardContent className="p-12 text-center">
              <h3 className="text-xl font-semibold text-gray-900 mb-2">
                No Nigerian Universities Offer This Course
              </h3>
              <p className="text-gray-600 mb-4">
                {course.name} is not currently offered as a standalone degree in Nigerian universities.
              </p>
              <p className="text-gray-600">
                Consider related courses or explore African and global universities.
              </p>
            </CardContent>
          </Card>
        )}
